...
public class CartDAO extends DAO {
    ...
    public List<Cart> getCartByAccount(int id){
        String sql = "select c.account_id, p.product_name, c.product_id, c.ammount, p.img, p.list_price, p.Discription from Cart as c join Product as p on c.product_id = p.product_id where c.account_id = ?";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                CartList.add(new Cart(aid, pName, pid, img, ammount, price, description));
            }
        } catch (Exception e) {
    ...
    public List<Cart> updateCart(int aid, int pid){
        String sql = "update Cart set ammount = ammount+1 where account_id = ? and product_id = ?";
        List<Cart> CartList = new ArrayList<Cart>();
         try {
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, aid);
